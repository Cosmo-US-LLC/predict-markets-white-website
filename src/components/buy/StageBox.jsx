...
const StageBox = () => {
  ...
  return (
    <div
      className="flex flex-col gap-2 rounded-[8px] px-[16px] py-[8px] bg-[#F5F5F5] h-[6.5rem] justify-center"
    ...
    >
      ...
      {!presaleEnded && <>
        {stage?.in_overflow_phase ? <>
          <div className="h-px bg-black/20 shrink-0" />
          <Countdown endDate={new Date(stage?.overflow_end ?? 0)} />
        </> : <>
          <div
            className="relative h-[10px] w-full rounded-full overflow-hidden isolate"
          >
